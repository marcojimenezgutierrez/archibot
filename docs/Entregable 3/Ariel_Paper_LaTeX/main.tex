...
animations, latency cues, and LLM-controlled emotional expressions. We report
...
web-native CVA (Three.js + VRM) over a RAG back end; (ii) a controlled
...
loading VRM models in the browser without an external game engine; this removes
the need for a Unity build and improves reproducibility. The
...
\textbf{Back end.}
A secure intermediary (ASP.NET) processes each query in three stages:
...
(OpenAI GPT-4o) together with behavior instructions; and (3)~\emph{response},
...
Audible narration uses the \textbf{Web Speech API}, removing external TTS
dependencies and reducing latency; \emph{thinking} animations cover retrieval
...
visibility, grid layout, automatic TTS and STT. In Condition~B (text) the layout
...
\node[box, fill=red!8, below=7mm of toggle] (back) {ASP.NET RAG\\Controller};
\node[box, fill=red!8, below left=6mm and -2mm of back] (vec) {Vector Store\\(file RAG)};
\node[box, fill=red!8, below right=6mm and -2mm of back] (llm) {GPT-4o\\(gen.\ + emotion)};
\draw[<->] (user) -- (app);
\draw[->] (app) -- (toggle);
\draw[->] (toggle) -- (avatar);
\draw[->] (avatar) -- (tts);
